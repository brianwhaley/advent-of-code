...
let countedOuterBags = []  ; 

async function countOuterBags(haystack, needle){
	var haystack2 = haystack.filter(function(elem, i, arr) {
    	var bag = Object.keys(elem)[0];
    	return elem[bag].toString().includes(needle);
	})
	// console.log(haystack2);
	for (var straws in haystack2) {
		var straw = Object.keys(haystack2[straws])[0];
		// console.log(straw);
		straw = straw.replace("bags", "bag");
		if ( ! countedOuterBags.includes(straw) ) countedOuterBags.push(straw) ; 
		await countOuterBags(haystack, straw) ;
	}
	return await countedOuterBags;
}
...
let countedInnerBags = 0 ; 

function countInnerBags(haystack, needle, parentCount){ 
	// find the bag passed in 
	var haystack2 = haystack.filter(function(elem, i, arr) {
    	var bag = Object.keys(elem)[0];
	...
	for (var straws in haystack2) {
		var straw = Object.keys(haystack2[straws])[0];
		for (var needleKey in haystack2[straws][straw]) {
			var needle = haystack2[straws][straw][needleKey] ;
			var bagCount = parseInt(needle.substring(0,needle.indexOf(" ")));
			if (isNaN(bagCount)) bagCount = 0 ; // "no other bags"
			var bagName = needle.substring(needle.indexOf(" ")+1);
			if (bagCount == 1) bagName = bagName.replace("bag", "bags");
			bagName = bagName.replace("bag.", "bags");
			bagName = bagName.replace("bags.", "bags");
			var newBags = (bagCount * parentCount) ;
			countedInnerBags += newBags ; 
			countInnerBags(haystack, bagName, newBags) ;
		}
	}
}
...
(function(){
	...
	var parsed = parseInput(inputs);
	console.log(parsed);
	countInnerBags(parsed, "shiny gold bag", 1) ;
	console.log("Counted Inner Bags = " + countedInnerBags);
})();
