...
function isValidPassport(pp){
	if(pp.byr && pp.iyr && pp.eyr && pp.hgt && pp.hcl && pp.ecl && pp.pid ) return true ;
	return false;
}

(function(){
	var validPassports = 0 ;
	var validStrictPassports = 0 ;
	for (var inputKey in inputs) {
		var pp = inputs[inputKey];
		 if (isValidPassport(pp)) {
		 	validPassports += 1;
		 	if (isValidStrictPassport(pp)){
    ...
    console.log("===== Part 2 =====");
	console.log("Valid Strict Passports = " + validStrictPassports);
})();
...
function isValidBYR(thisBYR){
	return ( thisBYR.length == 4 && parseInt(thisBYR) >= 1920 && parseInt(thisBYR) <= 2002 ) ;
}

function isValidIYR(thisIYR){
	return ( thisIYR.length == 4 && parseInt(thisIYR) >= 2010 && parseInt(thisIYR) <= 2020 ) ;
}

function isValidEYR(thisEYR){
	return ( thisEYR.length == 4 && parseInt(thisEYR) >= 2020 && parseInt(thisEYR) <= 2030 ) ;
}

function isValidHGT(thisHGT){
	var mySys = thisHGT.slice(-2) ;
	var myHGT = thisHGT.substr(0, thisHGT.length-2) ;
	if ( ( mySys != "cm" ) && ( mySys != "in" ) ) return false ;
	switch(mySys) {
  		case "cm":
    		return ( myHGT >= 150 && myHGT <= 193 ); break;
  		case "in":
    		return ( myHGT >= 59 && myHGT <= 76 ); break;
	}
	return true ;
}

function isValidHCL(thisHCL){
  	var hclPattern = /^#[0-9a-f]{6}/i ;
  	var hclResult = thisHCL.match(hclPattern);
	return (hclResult && hclResult.length > 0) ;
}

function isValidECL(thisECL){
	return ( ecls.includes(thisECL) ) ;
}

function isValidPID(thisPID){
	return ( thisPID.length == 9 ) ;
}

function isValidStrictPassport(pp){
	var validPid = ( pp.pid.length == 9 );
	if ( isValidBYR(pp.byr) && 
		isValidIYR(pp.iyr) && 
		isValidEYR(pp.eyr) && 
		isValidHGT(pp.hgt) && 
		isValidHCL(pp.hcl) &&
		isValidECL(pp.ecl) && 
		isValidPID(pp.pid) ) return true ;
	return false;
}
