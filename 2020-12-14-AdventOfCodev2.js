...
function applyBitmask(num, mask){
	let numBits = num.split("") ;
	let maskBits = mask.split("") ; //.reverse();
	for (i = 0; i < maskBits.length; i++ ){
		if(maskBits[i] != "X") { numBits[i] = maskBits[i] } ;
	}
	let numBitsStr = numBits.join("");
	return numBitsStr ; //(num == 0) ? (dec2bin(numBits)) : num ;
}
...
(function(){
	...
	let memory = [] ;
	let total = 0 ;
	let parsed = parseInput(inputs);
	for(let i = 0; i < parsed.length; i++) {
		let mask = parsed[i]["mask"];
		let mems = parsed[i]["mems"];
		for (let mem in mems) {
			let memval = dec2bin(mems[mem]);
...
function generateFloatingPossibilities(nums){
	...
	let pushed = false ;
	for(let num in nums){
		let thisNum = nums[num].split("") ;
		let thisX = thisNum.indexOf("X");
		if(thisX > -1){
			let floaters = [0, 1];
			for (let f in floaters){
				let tmp = [...thisNum]; // JSON.parse(JSON.stringify(newNum))
...
(function(){
	...
	let memory = [];
	let parsed = parseInput(inputs);
	for(let block in parsed) {
		let mask = parsed[block]["mask"];
		let mems = parsed[block]["mems"];
		for (let mem in mems) {
			let memAddr = dec2bin(mem);
			memAddr = applyFloatingBitmask(memAddr, mask);
			let memAddrs = generateFloatingPossibilities([memAddr]);
			let memAddrsDec = memAddrs.map( (val, i, arr ) => bin2dec(arr[i]) );
			for(let addr in memAddrsDec){
				memory[memAddrsDec[addr]] = mems[mem] ;
			}
		}
	}
	console.log(memory);
	console.log("Memory Complete");
	let total = Object.values(memory).reduce((a, b) => a + b, 0);
	console.log ("Total in memory = " + total);
})();
