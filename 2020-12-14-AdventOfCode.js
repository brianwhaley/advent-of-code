...
function parseInput(input){	
	...
	for (var i = 0; i < arr.length; i++) {
		...
		for (var key in arr[i]) {
			if (key != "mask" && key != "mems" ) { 
				var index = key.slice( key.indexOf("[") + 1, key.indexOf("]") );
				arr[i]["mems"][index] = parseInt(arr[i][key].toString());
				delete arr[i][key]; 
			}
		}
	}
...
function dec2bin(dec){
	...
	// return dec.toString(2).padStart(36, '0');
}
...
function applyBitmask(num, mask){
	...
	var maskbits = mask.split("") ; //.reverse();
	for (i = 0; i < maskbits.length; i++ ){
		if(maskbits[i] != "X") { newNum[i] = maskbits[i] } ;
	}
	var newNumStr = newNum.join("");
	return newNumStr ; //(num == 0) ? (dec2bin(newNum)) : num ;
}
...
(function(){
	...
	for(var i = 0; i < parsed.length; i++) {
		var mask = parsed[i]["mask"];
		var mems = parsed[i]["mems"];
		for (var mem in mems) {
			var memval = dec2bin(mems[mem]);
			memval = applyBitmask(memval, mask);
			memval = bin2dec(memval);
			memory[mem] = memval ;
		}
	}
	console.log(memory);
	total = memory.reduce(function(total, num){ return total + num ; })
	console.log ("Total in memory = " + total);
})();
// 23882525081 too low
// 9040426512135 too high
// 7817357407588 !!!
...
function applyFloatingBitmask(num, mask){
	var newNum = num.split("") ;
	var maskbits = mask.split("") ; //.reverse();
	for (var i in maskbits){
		switch(maskbits[i]) {
			/* case "0":
			  break; */  // Do Nothing
			case "1":
				newNum[i] = maskbits[i] ; break;
			case "X":
				newNum[i] = maskbits[i] ; break;
		  } 
	}
...
function generateFloatingPossibilities(nums){
	var newNums = [] ;
	let pushed = false ;
	for(var num in nums){
		var thisNum = nums[num].split("") ;
		var thisX = thisNum.indexOf("X");
		if(thisX > -1){
			var floaters = [0, 1];
			for (var f in floaters){
				var tmp = [...thisNum]; // JSON.parse(JSON.stringify(newNum))
				tmp[thisX] = floaters[f] ;
				newNums.push(tmp.join(""));
				pushed = true ;
			}
		}
	}
	if (pushed) { return generateFloatingPossibilities(newNums);
	} else { return nums; // no x left 
	}
}
...
(function(){
	...
	for(var block in parsed) {
		var mask = parsed[block]["mask"];
		var mems = parsed[block]["mems"];
		for (var mem in mems) {
			var memAddr = dec2bin(mem);
			memAddr = applyFloatingBitmask(memAddr, mask);
			var memAddrs = generateFloatingPossibilities([memAddr]);
			var memAddrsDec = memAddrs.map( (val, i, arr ) => bin2dec(arr[i]) );
			for(var addr in memAddrsDec){
				memory2[memAddrsDec[addr]] = mems[mem] ;
			}
		}
	}
	console.log(memory2);
	console.log("Memory Complete");
	var total = Object.values(memory2).reduce((a, b) => a + b, 0);
	console.log ("Total in memory = " + total);
})();
// 103113439646167 too high
// 4334683151052 == X
// 4335927555692
// 
...
// memory.sort(function(a,b){return a < b ;});
// memory2.sort(function(a,b){return a < b ;});

console.log(memory.length);
console.log(memory2.length);

console.log(Object.keys(memory).sort());

const memoryv2 = memory => Object.keys(memory).sort().reduce((obj, key) => (obj[key] = o[key], obj), {})
const memory2v2 = memory2 => Object.keys(memory2).sort().reduce((obj, key) => (obj[key] = o[key], obj), {})
...
console.log(diff(memory,memory2));

console.log(Object.keys(diff(memory,memory2)));

console.log(Object.keys(memory));
console.log(Object.keys(memory2));
