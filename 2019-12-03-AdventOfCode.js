...
function create_positions(wire){
	let pos = [ [0,0] ] ;
	for (i = 1; i < wire.length; i+=1) {
		var dir = wire[i].substring(0,1);
		var dist = parseInt(wire[i].substring(1, wire[i].length));
		switch(dir){
			case "U": 
				pos.push( [  pos[i-1][0], pos[i-1][1] + dist ] );
				break;
			case "D":
				pos.push( [  pos[i-1][0], pos[i-1][1] - dist ] );
				break;
			case "L":
				pos.push( [  pos[i-1][0] - dist, pos[i-1][1] ] );
				break ;
			case "R":
				pos.push( [  pos[i-1][0] + dist, pos[i-1][1] ] );
			// default:
